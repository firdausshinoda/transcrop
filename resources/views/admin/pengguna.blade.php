                    ...
                    <table id="dt_table" class="table table-striped" width="100%">
                        ...
                        @foreach($data as $key => $item)
                            <tr>
                                ...
                                <td><a href="{{url('/admin/pengguna_detail/'.$item->id_user)}}">{{$item->nama_user}}</a></td>
                                <td>{{$item->email_user}}</td>
                                <td>{{$item->alamat_user}}</td>
                                <td>{{$item->created_at}}</td>
                                <td>
                                    <button type="button" class="btn btn-sm btn-danger btn-block" onclick="ajax_delete('bank','{{$item->id_user}}')">HAPUS</button>
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
